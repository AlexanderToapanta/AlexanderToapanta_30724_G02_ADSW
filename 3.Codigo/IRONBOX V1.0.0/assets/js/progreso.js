(function () {
    ...
    atletaSelect.addEventListener('change', cargarHistorialSeleccionado);
    ...
    async function guardarResultado(evento) {
        ...
        const datos = {
            ...
            puntuacion: valorOpcional(puntuacionInput.value),
            notas: notasInput.value.trim(),
        };

        try {
            submitButton.disabled = true;
            await enviar('guardar', datos);
            mostrarEstado('Resultado WOD registrado correctamente.', 'ok');
            limpiarCampos(false);
            cerrarDrawer();
            await cargarHistorialSeleccionado();
        } catch (error) {
    ...
    async function cargarHistorialSeleccionado() {
        const idAtleta = Number(atletaSelect.value);
    ...
    function renderizarHistorial(registros) {
        ...
        emptyState.textContent = 'No hay registros para este atleta.';
    ...
    function limpiarCampos(restablecerAtleta) {
        const atletaActual = atletaSelect.value;
        form.reset();
        fechaInput.value = obtenerFechaActual();

        if (restablecerAtleta === false) {
